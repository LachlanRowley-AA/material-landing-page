...
export default async function DBM_ConcreteDataLoaderServer({
  accountKey,
  children,
  ...
  if (accountKey) {
    try {
      ...
      const response = await fetch(
        `https://script.google.com/macros/s/AKfycbw2rwVAaxZuk1JTkhfsKX0oM8MQgVopNa7YqvLzor0BoySL2zYVZa-t7E1kGq31m-SZ/exec?accountKey=${encodeURIComponent(accountKey)}`,
        { cache: 'no-store' }
      );
